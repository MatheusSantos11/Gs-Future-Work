        ...
        <div className="fixed top-16 left-0 w-full h-[calc(100vh-4rem)] bg-white dark:bg-[#202327] z-50 p-4 md:hidden overflow-y-auto animate-in slide-in-from-top-2 custom-scrollbar">
          <div className="flex flex-col min-h-full">
            ...
            <div className="mt-8 mb-6 space-y-6">
                ...
                {userCustom?.habilidadesTecnicas && getList(userCustom.habilidadesTecnicas).length > 0 && (<div><h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2"><Code size={14} className="text-blue-500"/> Habilidades</h3><div className="flex flex-wrap gap-2">{getList(userCustom.habilidadesTecnicas).map((tech, i) => (<span key={i} className="text-xs font-medium bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 px-2 py-1 rounded border border-blue-100 dark:border-blue-900/30 wrap-break-word">{tech}</span>))}</div></div>)}
                {userCustom?.softSkills && getList(userCustom.softSkills).length > 0 && (<div><h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2"><Star size={14} className="text-orange-500"/> Soft Skills</h3><div className="flex flex-wrap gap-2">{getList(userCustom.softSkills).map((skill, i) => (<span key={i} className="text-xs font-medium bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300 px-2 py-1 rounded-full wrap-break-word">{skill}</span>))}</div></div>)}
                {userCustom?.interesses && getList(userCustom.interesses).length > 0 && (<div><h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2"><Heart size={14} className="text-pink-500"/> Interesses</h3><div className="flex flex-wrap gap-2">{getList(userCustom.interesses).map((int, i) => (<span key={i} className="text-xs text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-white/5 px-2 py-1 rounded border border-gray-200 dark:border-transparent wrap-break-word">#{int}</span>))}</div></div>)}
